...
def parse(url: str) -> ParsedUrl:
    ...
    if url.startswith("spotify:"):
        parts = url.split(":")
        if len(parts) == 3 and parts[1] in {"track", "album", "artist"}:
            return ParsedUrl("spotify", parts[1], parts[2])
        raise UnsupportedUrlError(f"invalid spotify URI: {url}")

    try:
        parsed = urlparse(url)
    except Exception as e:
        raise UnsupportedUrlError(f"invalid URL: {url}") from e
    ...
    if host in {"music.youtube.com", "music.youtube.com"}:
        if path == "/watch":
    ...
    raise UnsupportedUrlError(f"no matching service: {url}")
```
...
Expected: Alle Tests grün (22 parametrisierte Fälle).
